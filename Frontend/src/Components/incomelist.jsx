...
const IncomeList = () => {
  ...
  const fetchIncomeData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://personalfinanceapp-production-3551.up.railway.app/api/userdata/${user._id}/income`);
  ...
  const handleDelete = async (incomeId) => {
    if (window.confirm("Are you sure you want to delete this income entry?")) {
      try {
        await axios.delete(`https://personalfinanceapp-production-3551.up.railway.app/api/userdata/${user._id}/income/${incomeId}`);
